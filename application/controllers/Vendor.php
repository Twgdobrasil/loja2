...
class Vendor extends CI_Controller
{
    ...
    function site_settings($para1 = "")
    {
     ...
     /* Clientes Favoritos */
    function clientes_favoritos($para1 = "")
    {
        if(!$this->crud_model->vendor_permission('clientes_favoritos')){
            redirect(base_url() . 'index.php/vendor');
        }
        
        //$page_data['tab_name']  = $para1;
		$page_data['page_name'] = "clientes_favoritos";
		
		$vendor_id = $this->session->userdata('vendor_id');
		$sql = "select u.user_id, username, email 
				from vendor_favoritos_user vfu, user u
				where vendor_id='".$vendor_id."' and vfu.user_id=u.user_id
				order by username";
		$query = $this->db->query($sql);
		$dados = $query->result_array();
		$page_data['dados'] = $dados;
		
		$this->load->view('back/index', $page_data);
    }

     /* Lista de Desejos */
    function lista_desejos($para1 = "")
    {
        if (!$this->crud_model->vendor_permission('lista_desejos')) {
            redirect(base_url() . 'index.php/vendor');
        }
        $page_data['page_name'] = "lista_desejos";
        $page_data['tab_name']  = $para1;
        $this->load->view('back/index', $page_data);
    }

     /* Comprar Créditos */
    function comprar_creditos($operacao = '',$plano_id = ''){
        if (!$this->crud_model->vendor_permission('comprar_creditos')) {
            redirect(base_url() . 'index.php/vendor');
        }
		
		if($operacao == 'traz_infos_plano'){
			
			$sql    = "select *
					   from planos_creditos
					   where plano_creditos_id='".$plano_id."'";
			$query = $this->db->query($sql);
			$dados = $query->result_array();
			...
			$total = count($dados);
			if($total > 0){
				$dados_f .= $dados[$cont]['nome'].'|';
				$dados_f .= $dados[$cont]['validade'].'|';
				$dados_f .= $dados[$cont]['valor_por_venda'].'|';
				$dados_f .= $dados[$cont]['qtd_vendas'].'|';
				$dados_f .= $dados[$cont]['preco'];
			}
			
			die($dados_f);
		}elseif($operacao == 'grava_dados'){
			
			$vendor_id = $this->session->userdata('vendor_id');
			
			$data['vendor_id']         = $vendor_id;
			$data['plano_creditos_id'] = $plano_id;
			$data['qtd_max_plano']     = $this->input->post('qtd');
			$data['tipo_pagamento']    = $this->input->post('tipo_pagamento');
			$data['data_compra_plano'] = date('Y-m-d 00:00');
			$data['data_expira_plano'] = date('Y-m-d',strtotime(' + '.$this->input->post('qtd_dias').' days'));
			
			if($this->db->insert('plano_creditos_vendor', $data)){
				die('1');
			}else{
				die('-1');	
			}
			...
			$page_data['page_name'] = "comprar_creditos";
			//$page_data['tab_name']  = $para1;
			$this->load->view('back/index', $page_data);
		}
	}
     ...
     /* Manage Site Settings */
    function chat($operacao = ""){
        /*if (!$this->crud_model->vendor_permission('chat')) {
            redirect(base_url() . 'index.php/vendor');
        }*/
		if($operacao == 'traz_usuarios_vendor'){
			
			$sql = "select u.username, u.user_id, token_cnll as token
					from user u, vendor_favoritos_user vu
					where vu.vendor_id=".$this->session->userdata('vendor_id')." and vu.user_id=u.user_id and u.username like '%".$_GET['valor']."%'
					order by u.username";
			$query = $this->db->query($sql);
			$dados = $query->result_array();
			...
			if($total > 0){
				while($cont<$total){
					$dados_user .= $dados[$cont]['user_id'].'|';
					$dados_user .= $dados[$cont]['username'].'|';
					$dados_user .= $dados[$cont]['token'].'|';
					if($this->url_exists('http://www.clicknalupa.com.br/loja/uploads/user_image/user_'.$dados[$cont]['user_id'].'.jpg')){
					    $tem_img = '1';
				    }else{
					    $tem_img = '-1';
				    }
					$dados_user .= $tem_img.'{S}';
					$cont++;
				}
				$dados_user = substr($dados_user,0,-3);
			}
			
			die($dados_user);
		}elseif($operacao == 'envia_mensagem'){
			
			$user_id   = $_GET['user_id'];
			$vendor_id = $this->session->userdata('vendor_id');
			$mensagem  = json_decode($_GET['mensagem']);
			
			$data['enviada_por'] = 'vendor';
			$data['user_id']     = $user_id;
			$data['vendor_id']   = $vendor_id;
			$data['mensagem']    = $mensagem;
			$data['data'] 		 = date('Y-m-d H:i:s');
			
			$this->db->insert('mensagens_cnll', $data);
			die();
		}elseif($operacao == 'traz_conversa_usuario'){
			...
			if($total > 0){
				while($cont<$total){
					...
					$dados_msg .= $dados[$cont]['mensagem'].'{S}';
					$cont++;
				}
			...
			$sql = "select vu.vendor_id, u.username, u.user_id, token_cnll as token
					from user u, vendor_favoritos_user vu
					where vu.vendor_id=".$this->session->userdata('vendor_id')." and vu.user_id=u.user_id
					order by u.username";
			$query = $this->db->query($sql);
			$page_data['dados'] = $query->result_array();
			$this->load->view('back/index', $page_data);	
		}
	}
	
	//Testa se uma iamgem passada com url existe
    ...
    function package($para1 = "", $para2 = "")
    {
        if ($para1 == 'upgrade') {
            ...
            if($type !== '0'){
                ...
                $amount_in_usd  = $amount/exchange('def');
                if ($method == 'paypal') {
