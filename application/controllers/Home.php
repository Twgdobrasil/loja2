...
class Home extends CI_Controller
{
    ...
    function vendor_logup($para1 = "", $para2 = "")
    {
        if ($para1 == "add_info") {
            ...
            //Recebe os valores de horario de funcionamento
            $h_i_s = $this->input->post('horario_semana_inicio');
            $h_f_s = $this->input->post('horario_semana_fim');
            $h_i_f = $this->input->post('horario_fds_inicio');
            $h_f_f = $this->input->post('horario_fds_fim');
            ...
            if ($this->form_validation->run() == FALSE){
				...
				if( $h_i_s != '' && (!is_numeric(substr($h_i_s,0,2)) || !is_numeric(substr($h_i_s,3,2))) ){ 
					echo 'Horario Abertura Dia Semana: Formato incorreto.';	
				}else{
					if( $h_f_s != '' && (!is_numeric(substr($h_f_s,0,2)) || !is_numeric(substr($h_f_s,3,2))) ){ 
						echo 'Horario Fechamento Dia Semana: Formato incorreto.';
					}else{
						if( $h_i_f != '' && (!is_numeric(substr($h_i_f,0,2)) || !is_numeric(substr($h_i_f,3,2))) ){ 
							echo 'Horario Abertura Fim Semana: Formato incorreto.';
						}else{
							if( $h_f_f != '' && (!is_numeric(substr($h_f_f,0,2)) || !is_numeric(substr($h_f_f,3,2))) ){ 
								echo 'Horario Fechamento Fim Semana: Formato incorreto.';
							}else{
								if($safe == 'yes'){
									...
									$data['horario_semana_inicio'] = $this->input->post('horario_semana_inicio');
									$data['horario_semana_fim']    = $this->input->post('horario_semana_fim');
									$data['horario_fds_inicio']    = $this->input->post('horario_fds_inicio');
									$data['horario_fds_fim']       = $this->input->post('horario_fds_fim');
