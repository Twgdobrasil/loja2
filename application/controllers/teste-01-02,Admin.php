...
class Admin extends CI_Controller
{
   ...
   /* Manage Frontend Captcha Settings Credentials */
	...
	function propagandas($operacao = '',$propaganda_id = ''){
		if (!$this->crud_model->admin_permission('vendor')) {
            redirect(base_url() . 'index.php/admin');
        }
		if($operacao == 'atualiza_propaganda'){
			
            $data['link'] 			 = empty($this->input->post('link')) ? '' : $this->input->post('link');
			$data['nome_fornecedor'] = empty($this->input->post('nome_fornecedor')) ? '' : $this->input->post('nome_fornecedor');
            if($this->input->post('check_publicar') == 'ok'){
                $publicar = '1';
			}else{
                $publicar = '0';
            }
			...
			//Arquivo Fundo
			if($_FILES['imagem_upload']['name']){
				$dados_imagem 	 = explode('.',$_FILES['imagem_upload']['name']);
				$extensao_imagem = '.'.$dados_imagem[1];
				$hash            = substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVXZ0123456789'),0,4);
				$imagem			 = $hash.$propaganda_id.$extensao_imagem;
				if(!move_uploaded_file($_FILES['imagem_upload']['tmp_name'], 'uploads/propagandas/'.$imagem)){
					die('-1');	
				}
			}else{
				$imagem = '';
			...
			$this->db->where('propaganda_id',$propaganda_id);	
			$this->db->update('propagandas', $data);
			
		}elseif($operacao == 'publica_propaganda'){
			
            $data['publicar'] = $this->input->post('publicar');
			$this->db->where('propaganda_id',$propaganda_id);	
			$this->db->update('propagandas', $data);

		}else{
        	$page_data['page_name'] = "propagandas";
			...
			$dados_propaganda = $query->result_array();
			
            //Enviar os dados certinhos
            //$dados[ID]['ID']              = ID
            //$dados[ID]['LINK']            = LINK
            //$dados[ID]['NOME_FORNECEDOR'] = NOME_FORNECEDOR
            //$dados[ID]['PUBLICADO']       = PUBLICADO
            //$dados[ID]['IMAGEM']          = IMAGEM
            $cont  = 0;
            $total = count($dados_propaganda);
			if($total > 0){
				while($cont<$total){
					$dados[$dados_propaganda[$cont]['propaganda_id']]['ID']              = $dados_propaganda[$cont]['propaganda_id']; 
					$dados[$dados_propaganda[$cont]['propaganda_id']]['LINK']            = $dados_propaganda[$cont]['link']; 
					$dados[$dados_propaganda[$cont]['propaganda_id']]['NOME_FORNECEDOR'] = $dados_propaganda[$cont]['nome_fornecedor']; 
					$dados[$dados_propaganda[$cont]['propaganda_id']]['PUBLICADO']       = $dados_propaganda[$cont]['publicar']; 
					$dados[$dados_propaganda[$cont]['propaganda_id']]['IMAGEM']          = $dados_propaganda[$cont]['imagem']; 
					$cont++;
				}
			}
    ...
    function creditos($operacao = ''){
        ...
        if($operacao == ''){
            ...
            //QUERY
            $sql = "select pcv.plano_creditos_vendor_id, pcv.status, pcv.qtd_vendas_feitas, pcv.qtd_max_plano 
                           v.vendor_id, v.name, v.display_name, v.email, 
                           pc.nome
                    from plano_creditos_vendor pcv, vendor v, planos_creditos pc
                    where pcv.vendor_id=v.vendor_id and pcv.plano_creditos_id = pc.plano_creditos_id
                    order by propaganda_id";
            $query = $this->db->query($sql);
            ...
            $this->load->view('back/index', $page_data);
        }elseif($operacao == 'atualiza_status'){
            $data['status'] = $this->input->post('status');
            $this->db->where('plano_creditos_vendor_id',$this->input->post('id'));   
            $this->db->update('plano_creditos_vendor', $data);
        }
    }

    function vendas_report($para1 = '', $para2 = ''){
        if (!$this->crud_model->admin_permission('vendor')) {
            redirect(base_url() . 'index.php/admin');
        }
        $page_data['page_name'] = "vendas_report";
        $this->load->view('back/index', $page_data);
    }
